...
shinyServer(function(input, output, session) {
    ...
    createPlotUI <- function(df, input, var, var2, varFill, theme, palette,  var1Num, var2Num, var1count) {
    
      if(input$inputSelect_Var2 != "keine" & !var2Num & !var1Num & input$inputCheck_Sankey) {
        
        fluidRow(
          output$sankeyPlotGnerator <- renderSankeyNetwork({
            generateSankeyPlot(df, var, var2)
          })
        )
    ...
    output$plotUI <- renderUI({
      # dynamischer Plot, abhängig vom Datentyp der ausgewählten Variable
        ...
        #createPlotUI()
        createPlotUI(df, input, var, var2, varFill, theme, palette,  var1Num, var2Num, var1count)
    })
